...
/*   Updated: 2020/12/21 09:39:46 by archid-          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	adjust_base_prefix(char **astr, t_frmt *frmt, bool replace, bool insert)
{
	...
	status = false;
	if (!(astr && *astr) || !frmt ||
			(!ft_has_flag(frmt, FL_HASH) && frmt->conv != CONV_PTR))
		return (status);
	prefix = NULL;
	if (ft_is_hex(frmt, frmt->conv))
		prefix = frmt->is_upcase ? "0X" : "0x";
	else if (frmt->conv == CONV_UBIN && ft_has_flag(frmt, FL_HASH))
...
bool	flag_alterform(t_frmt *frmt, char **astr, size_t *pad)
{
	if (!(astr && *astr) || !frmt || !pad
			|| ((!ft_has_flag(frmt, FL_HASH) || !(format_isnumeric(frmt)))
					&& frmt->conv != CONV_PTR))
		return (false);
	...
	else if ((frmt->conv == CONV_UHEX && ft_has_flag(frmt, FL_HASH)
...
void	flag_zero_padding(t_frmt *frmt, char **astr, size_t *pad)
{
	...
	{
		if (tmp_sign == '+' && !ft_has_flag(frmt, FL_PLUS)
				&& ft_has_flag(frmt, FL_SPACE))
			tmp_sign = ' ';
